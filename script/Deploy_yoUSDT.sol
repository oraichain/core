...
contract Deploy is BaseScript {
    /// Deploy yoUSDT từ env. Gọi: forge script script/Deploy_yoUSDT.sol:Deploy run
    /// Bắt buộc: VAULT_NAME, VAULT_SYMBOL, VAULT_ASSET, VAULT_OWNER
    /// Tùy chọn: VAULT_AUTHORITY, VAULT_IMPL (0), VAULT_DEPOSIT_AMOUNT (0), VAULT_PAUSE (false)
    function run() public returns (yoUSDT vault, RolesAuthority authority) {
        return run(
    ...
    function run(
        ...
        public
        broadcast
        returns (yoUSDT vault, RolesAuthority authority)
    {
        console.log("Deploying yoUSDT...");
        console.log("Name: ", _name);
        console.log("Symbol: ", _symbol);
        console.log("Asset: ", _asset);
        console.log("Owner: ", _owner);
        console.log("Authority: ", _authority);
        console.log("Vault (impl reuse): ", _vault);
        ...
        if (_vault == address(0)) {
            impl = new yoUSDT();
            console.log("yoUSDT implementation deployed at: ", address(impl));
        } else {
            impl = yoUSDT(payable(_vault));
        }

        bytes memory data = abi.encodeWithSelector(yoUSDT.initialize.selector, IERC20(_asset), _owner, _name, _symbol);
        ...
        console.log("yoUSDT proxy deployed at: ", address(vault));
        ...
        console.log("Withdraw Fee:", vault.feeOnWithdraw());
