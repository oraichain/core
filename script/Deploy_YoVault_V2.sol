...
contract Deploy is BaseScript {
    /// Deploy YoVault_V2 từ env. Gọi: forge script script/Deploy_YoVault_V2.sol:Deploy run
    ...
    function run() public returns (YoVault_V2 vault, RolesAuthority authority) {
        return run(
    ...
    {
        console.log("Deploying YoVault_V2...");
        ...
        if (_vault == address(0)) {
            impl = new YoVault_V2();
            console.log("YoVault_V2 implementation deployed at: ", address(impl));
        } else {
            impl = YoVault_V2(payable(_vault));
        }

        bytes memory data = abi.encodeWithSelector(YoVault_V2.initialize.selector, IERC20(_asset), _owner, _name, _symbol);
        ...
        console.log("YoVault_V2 proxy deployed at: ", address(vault));
